...
/* ──────────────────────────── PMQuest — sketch wireframe entry ─────────── */

const PMQ_TABS = [
  { id: 'p1', num: '01', label: 'Classic sidebar dashboard' },
  { id: 'p2', num: '02', label: 'Roadmap / путь героя' },
  { id: 'p3', num: '03', label: 'Daily Mission first' },
  { id: 'p4', num: '04', label: 'Bento grid' },
  { id: 'p5', num: '05', label: 'Mechanics · приколюхи' },
];

const PMQuestScreen = ({ onPickTrack, onOpenLearn, onOpenInterview, onOpenQuiz, tracks }) => {
  const [tab, setTab] = useState('p2');
  const productTrack = tracks.find((t) => t.id === 'product') || tracks[0];
  const startMock = () => productTrack && onPickTrack(productTrack);

  return (
    <div className="pmq">
      <header className="pmq-page-header">
        <div className="crumbs">
          <span className="mono">PROJECT</span>
          <span>•</span>
          <span className="mono">PMQUEST — EDTECH</span>
          <span>•</span>
          <span className="mono">ENTRY DASHBOARD — WIREFRAMES v1</span>
        </div>
        <h1>Дашборд после входа <span className="underline-scribble">— 4 направления</span></h1>
        <p style={{ margin: 0, color: 'var(--pmq-ink-2)', maxWidth: '90ch' }}>
          Аудитория: джуны на первом PM-интервью. Тон: дружелюбный, игровой (XP / стрики / уровни).
          AI — помощник, не центр. Каждый вариант — про разный «первый взгляд».
        </p>
      </header>

      <nav className="pmq-tabs" role="tablist">
        {PMQ_TABS.map((t) => (
          <button
            key={t.id}
            className={`pmq-tab${tab === t.id ? ' active' : ''}`}
            onClick={() => setTab(t.id)}
            role="tab"
      ...
      <main className="pmq-panels">
        {tab === 'p1' && <PMQPanel1 startMock={startMock} onOpenLearn={onOpenLearn} />}
        {tab === 'p2' && <PMQPanel2 startMock={startMock} onOpenLearn={onOpenLearn} onOpenInterview={onOpenInterview} />}
        {tab === 'p3' && <PMQPanel3 startMock={startMock} onOpenLearn={onOpenLearn} />}
        {tab === 'p4' && <PMQPanel4 startMock={startMock} onOpenLearn={onOpenLearn} onOpenInterview={onOpenInterview} onOpenQuiz={onOpenQuiz} />}
        {tab === 'p5' && <PMQPanel5 startMock={startMock} onOpenInterview={onOpenInterview} />}
      </main>
    </div>
  );
};

/* W1 — Classic sidebar dashboard */
function PMQPanel1({ startMock, onOpenLearn }) {
  return (
    <section>
      <div style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div className="label-tag">VARIATION 01 / SAFE BET</div>
          <h2 className="hand" style={{ margin: '2px 0 0', fontSize: 30 }}>Классический дашборд — «всё на месте»</h2>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-mint)' }} />знакомо</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-yellow)' }} />много опций сразу</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-pink)' }} />проще навигация</span>
        </div>
      </div>

      <div className="pmq-frame pmq-w1">
        <span className="pmq-frame-caption">01 · Classic dashboard</span>
        <span className="pmq-frame-note">first impression: «у меня есть всё»</span>
        ...
        <aside className="sidebar">
          ...
          <div className="nav-item active"><span className="ico">⌂</span> Home</div>
          <div className="nav-item" onClick={onOpenLearn}><span className="ico">▦</span> My plan</div>
          <div className="nav-item" onClick={startMock}><span className="ico">◎</span> Mock interview</div>
          <div className="navgroup-label">Study</div>
          <div className="nav-item" onClick={onOpenLearn}><span className="ico">?</span> Question bank</div>
          <div className="nav-item" onClick={startMock}><span className="ico">▤</span> Cases</div>
          <div className="nav-item"><span className="ico">★</span> Behavioral / STAR</div>
          <div className="nav-item"><span className="ico">⌬</span> System design</div>
          <div className="nav-item"><span className="ico">∑</span> Analytics &amp; SQL</div>
          <div className="nav-item" onClick={onOpenLearn}><span className="ico">≡</span> Theory library</div>
          <div className="navgroup-label">Tools</div>
          <div className="nav-item"><span className="ico">✎</span> Resume review</div>
          <div className="nav-item"><span className="ico">◈</span> AI tutor</div>
          <div className="nav-item"><span className="ico">⚑</span> Roadmap</div>
          <div style={{ marginTop: 'auto', paddingTop: 14, borderTop: '1.5px dashed var(--pmq-ink-4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div className="pmq-avatar" style={{ width: 30, height: 30, fontSize: 14 }}>EК</div>
              <div style={{ fontFamily: "'Patrick Hand', cursive", lineHeight: 1.1 }}>
                <div>Ekaterina</div>
                <div className="mono">Lvl 4 · Junior PM</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <div className="main">
          <div className="topbar">
            <div className="search"><span className="mono">⌕</span> Поиск кейсов, тем, компаний…</div>
            <div className="stat"><span className="dot" style={{ background: 'var(--pmq-hl-coral)' }}>🔥</span>7 дней</div>
            <div className="stat"><span className="dot" style={{ background: 'var(--pmq-hl-purple)' }}>✦</span>1 240 XP</div>
            <div className="stat"><span className="dot" style={{ background: 'var(--pmq-hl-yellow)' }}>★</span>Lvl 4</div>
            <div className="pmq-avatar">EК</div>
          </div>

          <div className="hero">
            <div>
              <div className="label-tag">ПОНЕДЕЛЬНИК · 6 НЕДЕЛЯ ПОДГОТОВКИ</div>
              <h2>Привет, Катя 👋</h2>
              <p>До твоего таргета (Junior PM @ FAANG) осталось <b>54 дня</b>. План на сегодня — 35 минут.</p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button className="pmq-btn primary" onClick={startMock}>▶ Продолжить с того же места</button>
                <button className="pmq-btn" onClick={startMock}>+ Новый mock с AI</button>
                <button className="pmq-btn ghost" onClick={onOpenLearn}>План недели →</button>
              </div>
            </div>
            <div className="pmq-ph">Иллюстрация / маскот<br />(приветствие)</div>
          </div>

          <div className="grid-2">
            <div className="card">
              <h3>План на сегодня <small>35 мин · 3 шага</small></h3>

              <div className="pmq-task done">
                <div className="check">✓</div>
                <div>
                  <div className="title">Урок: Что такое «product sense»</div>
                  <div className="mono">theory · 8 мин · +20 XP</div>
                </div>
                <span className="pmq-chip" style={{ background: 'var(--pmq-hl-mint)' }}>done</span>
                <button className="pmq-btn sm ghost">обзор</button>
              </div>

              <div className="pmq-task">
                <div className="check" />
                <div>
                  <div className="title">Кейс: «Дизайн фичи лайков для Spotify»</div>
                  <div className="mono">cases · 15 мин · +60 XP · FAANG-style</div>
                </div>
                <span className="pmq-chip" style={{ background: 'var(--pmq-hl-yellow)' }}>now</span>
                <button className="pmq-btn sm primary" onClick={startMock}>старт</button>
              </div>

              <div className="pmq-task">
                <div className="check" />
                <div>
                  <div className="title">Behavioral mock: «Tell me about a time…»</div>
                  <div className="mono">mock · 12 мин · +40 XP · с AI-интервьюером</div>
                </div>
                <span className="pmq-chip">queued</span>
                <button className="pmq-btn sm ghost">позже</button>
              </div>

              <hr className="pmq-hr-dash" />

              <h3 style={{ marginTop: 8 }}>Прогресс по темам <small>9 разделов</small></h3>

              {[
                { name: 'Product design / Cases', pct: 62, color: 'var(--pmq-accent)' },
                { name: 'Behavioral / STAR', pct: 38, color: 'var(--pmq-accent-2)' },
                { name: 'System design for PM', pct: 18, color: 'var(--pmq-hl-coral)' },
                { name: 'Analytics & A/B', pct: 44, color: 'var(--pmq-hl-purple)' },
                { name: 'Estimation / guesstimates', pct: 55, color: 'var(--pmq-hl-mint)' },
              ].map((t) => (
                <div className="pmq-topic-row" key={t.name}>
                  <div className="name">{t.name}</div>
                  <div className="pmq-bar"><i style={{ width: `${t.pct}%`, background: t.color }} /></div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div className="card" style={{ background: 'var(--pmq-hl-yellow)' }}>
                <h3 style={{ margin: '0 0 4px' }}>🔥 Стрик · 7 дней</h3>
                <div className="mono" style={{ marginBottom: 8 }}>не пропусти сегодня</div>
                <div className="pmq-streak-grid">
                  {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                    <div key={i} className={`day ${i === 6 ? 'today' : 'on'}`}>{d}</div>
                  ))}
                </div>
              </div>
              ...
              <div className="card">
                <h3>Цель</h3>
                <div style={{ fontFamily: "'Patrick Hand', cursive", fontSize: 16, marginBottom: 6 }}>Junior PM @ FAANG · 17 июля</div>
                <div className="pmq-bar" style={{ marginBottom: 6 }}><i style={{ width: '46%' }} /></div>
                <div className="mono">46% готовности · 54 дня</div>
                <hr className="pmq-hr-dash" />
                <div className="mono" style={{ marginBottom: 4 }}>слабые места:</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <span className="pmq-chip" style={{ background: 'var(--pmq-hl-pink)' }}>System design</span>
                  <span className="pmq-chip" style={{ background: 'var(--pmq-hl-coral)' }}>Estimation</span>
                </div>
              </div>
              ...
              <div className="card">
                <h3>Ачивки</h3>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <div className="pmq-ph" style={{ width: 48, height: 48, borderRadius: '50%' }}>🏅</div>
                  <div className="pmq-ph" style={{ width: 48, height: 48, borderRadius: '50%' }}>⚡</div>
                  <div className="pmq-ph" style={{ width: 48, height: 48, borderRadius: '50%' }}>📘</div>
                  <div className="pmq-ph" style={{ width: 48, height: 48, borderRadius: '50%', borderStyle: 'dashed' }}>?</div>
                  <div className="pmq-ph" style={{ width: 48, height: 48, borderRadius: '50%', borderStyle: 'dashed' }}>?</div>
                </div>
                <div className="mono" style={{ marginTop: 8 }}>3 / 24 открыто</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <p className="hand" style={{ fontSize: 20, margin: '18px 0 0', color: 'var(--pmq-ink-2)', maxWidth: '60ch' }}>
        ➜ Сильные стороны: пользователь видит всё разом — план, прогресс, стрик, цель.
        Минус: на первый экран много всего, страх «потеряться».
      </p>
    </section>
  );
}
...
/* W2 — Roadmap */
function PMQPanel2({ startMock, onOpenLearn, onOpenInterview }) {
  const stages = [
    {
      num: 1, title: 'Foundations', tag: '— что такое PM, фреймворки',
      nodes: [
        { ico: '★', label: 'Что такое PM', name: 'intro', state: 'done' },
        { ico: '★', label: 'CIRCLES, AARM', name: 'frame works', state: 'done' },
        { ico: '★', label: 'Метрики 101', name: 'metrics', state: 'done' },
        { ico: '★', label: 'Mini-quiz', name: 'quiz', state: 'done' },
      ],
    },
    {
      num: 2, title: 'Product design кейсы', tag: '— любимая часть FAANG',
      nodes: [
        { ico: '▤', label: 'Гайд', name: 'guide', state: 'done' },
        { ico: '▤', label: 'Spotify likes', name: 'case 1', state: 'done' },
        { ico: '▤', label: '→ Google Maps for kids', name: 'case 2', state: 'current', badge: 'сейчас' },
        { ico: '▤', label: 'Airbnb hosts', name: 'case 3', state: '' },
        { ico: '⚑', label: 'Mini-mock', name: 'boss', state: '' },
      ],
    },
    {
      num: 3, title: 'Behavioral / STAR', tag: '— про себя и команду',
      nodes: [
        { ico: '★', label: 'метод', name: 'STAR', state: '' },
        { ico: '★', label: 'FAANG банк', name: '30 вопросов', state: '' },
        { ico: '◎', label: 'AI-интервью', name: 'mock', state: '' },
        { ico: '⚑', label: 'с ментором', name: 'boss', state: '' },
      ],
    },
    {
      num: 4, title: 'System design + Analytics', tag: '— заблокировано до конца 2-го этапа',
      nodes: [
        { ico: '⌬', label: 'пока ⛌', name: 'basics', state: 'locked' },
        { ico: '⌬', label: 'пока ⛌', name: 'design', state: 'locked' },
        { ico: '∑', label: 'пока ⛌', name: 'SQL', state: 'locked' },
        { ico: '∑', label: 'пока ⛌', name: 'A/B', state: 'locked' },
      ],
    },
  ];
  ...
  return (
    <section>
      <div style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div className="label-tag">VARIATION 02 / GAMIFIED</div>
          <h2 className="hand" style={{ margin: '2px 0 0', fontSize: 30 }}>Путь героя — карта уровней</h2>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-coral)' }} />сильная геймификация</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-mint)' }} />понятный «следующий шаг»</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-sky)' }} />фокус на прогрессе</span>
        </div>
      </div>

      <div className="pmq-frame pmq-w2">
        <span className="pmq-frame-caption">02 · Roadmap / journey</span>
        <span className="pmq-frame-note">first impression: «вот мой путь»</span>

        <div className="header">
          <div className="left">
            <div className="logo">
              <span style={{ display: 'inline-flex', width: 32, height: 32, border: 'var(--pmq-border)', borderRadius: 8, background: 'var(--pmq-accent)', color: 'white', alignItems: 'center', justifyContent: 'center', fontFamily: "'JetBrains Mono', monospace", fontSize: 14, boxShadow: 'var(--pmq-shadow-hard-sm)' }}>P!</span>
              PMQuest
            </div>
            <div className="nav-pills">
              <button className="pmq-btn sm" style={{ background: 'var(--pmq-accent)', color: 'white' }}>Мой путь</button>
              <button className="pmq-btn sm ghost" onClick={onOpenInterview}>Mock-комната</button>
              <button className="pmq-btn sm ghost" onClick={onOpenLearn}>Библиотека</button>
              <button className="pmq-btn sm ghost">CV ревью</button>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div className="pmq-chip" style={{ background: 'var(--pmq-hl-coral)' }}>🔥 7</div>
            <div className="pmq-chip">✦ 1 240 XP</div>
            <div className="pmq-avatar">EК</div>
          </div>
        </div>
          ...
          <div className="map-area">
            {stages.map((stage) => (
              <React.Fragment key={stage.num}>
                ...
                <div className="path-row" style={stage.num === 4 ? { opacity: 0.7 } : undefined}>
                  {stage.nodes.map((n, i) => (
                    <React.Fragment key={n.name + i}>
                      <div className={`node ${n.state}`} onClick={n.state === 'current' ? startMock : undefined} style={{ cursor: n.state === 'current' ? 'pointer' : 'default' }}>
                        <div className="ico">{n.ico}</div>
                        {n.name}
                        {n.badge && <div className="badge">{n.badge}</div>}
                        <div className="node-label">{n.label}</div>
                      </div>
                      {i < stage.nodes.length - 1 && <div className="path-connector" />}
                    </React.Fragment>
                  ))}
                </div>
              </React.Fragment>
            ...
            <div className="current-card">
              <div>
                <div className="tag-row">
                  <span className="pmq-chip" style={{ background: 'var(--pmq-accent)', color: 'white', borderColor: 'var(--pmq-ink)' }}>▶ следующий шаг</span>
                  <span className="pmq-chip">кейс · 15 мин</span>
                  <span className="pmq-chip">+60 XP</span>
                  <span className="pmq-chip" style={{ background: 'var(--pmq-hl-pink)' }}>FAANG-style</span>
                </div>
                <h2>Google Maps для детей 8–12 лет</h2>
                <p>Спроектируй фичу. Шаги: clarifying questions → user → pain → solutions → метрики. Подсказки спрятаны, AI-тутор подключится по запросу.</p>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <button className="pmq-btn primary" onClick={startMock}>▶ Начать кейс</button>
                  <button className="pmq-btn">Посмотреть пример решения</button>
                  <button className="pmq-btn ghost">Пропустить</button>
                </div>
              </div>
              <div className="pmq-ph" style={{ height: 140 }}>Иллюстрация уровня</div>
            </div>
          </div>
          ...
          <aside className="sidepanel">
            <div className="profile-card">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div className="pmq-avatar">EК</div>
                <div>
                  <div style={{ fontFamily: "'Caveat', cursive", fontSize: 22 }}>Катя · Lvl 4</div>
                  <div className="mono">junior pm · 6-я неделя</div>
                </div>
              </div>
              <div className="pmq-bar"><i style={{ width: '62%' }} /></div>
              <div className="mono" style={{ marginTop: 4 }}>62% до Lvl 5 · +260 XP осталось</div>
            </div>

            <div className="profile-card">
              <div className="row"><span>🔥 Стрик</span> <b>7 дней</b></div>
              <div className="row"><span>✦ XP сегодня</span> <b>+20</b></div>
              <div className="row"><span>🏅 Ачивки</span> <b>3 / 24</b></div>
              <div className="row"><span>🎯 Целевая дата</span> <b>17 июля</b></div>
            </div>

            <div className="profile-card" style={{ background: 'var(--pmq-hl-mint)' }}>
              <div style={{ fontFamily: "'Caveat', cursive", fontSize: 22, marginBottom: 6 }}>AI-тутор Джо</div>
              <div style={{ fontFamily: "'Patrick Hand', cursive", fontSize: 15, marginBottom: 10 }}>
                «Заметил, что ты быстро проходишь Foundations. Может, попробуем mock?»
              </div>
              <button className="pmq-btn sm" onClick={startMock}>ответить →</button>
            </div>

            <div className="profile-card">
              <div style={{ fontFamily: "'Caveat', cursive", fontSize: 22, marginBottom: 6 }}>Сегодня в комьюнити</div>
              <div className="mono" style={{ marginBottom: 6 }}>3 джуна решают тот же кейс</div>
              <div style={{ display: 'flex' }}>
                <div className="pmq-avatar" style={{ width: 28, height: 28, fontSize: 14, background: 'var(--pmq-hl-yellow)', marginRight: -6 }}>М</div>
                <div className="pmq-avatar" style={{ width: 28, height: 28, fontSize: 14, background: 'var(--pmq-hl-sky)', marginRight: -6 }}>А</div>
                <div className="pmq-avatar" style={{ width: 28, height: 28, fontSize: 14, background: 'var(--pmq-hl-pink)' }}>Л</div>
              </div>
            </div>
          </aside>
        </div>
      ...
      <p className="hand" style={{ fontSize: 20, margin: '18px 0 0', color: 'var(--pmq-ink-2)', maxWidth: '80ch' }}>
        ➜ Сильно: всегда понятно «что дальше», прогресс осязаем, мотивирует. Минус: труднее свободно «попрыгать» по темам.
      </p>
    </section>
  );
}
...
/* W3 — Daily Mission */
function PMQPanel3({ startMock, onOpenLearn }) {
  return (
    <section>
      <div style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div className="label-tag">VARIATION 03 / FOCUSED</div>
          <h2 className="hand" style={{ margin: '2px 0 0', fontSize: 30 }}>«Миссия дня» — одно главное действие</h2>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-coral)' }} />низкое сопротивление</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-yellow)' }} />«15 минут в день»</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-mint)' }} />минимум выбора</span>
        </div>
      </div>

      <div className="pmq-frame pmq-w3">
        <span className="pmq-frame-caption">03 · Daily mission</span>
        <span className="pmq-frame-note">first impression: «сделай это и закрой вкладку»</span>

        <div className="topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
            <div className="logo" style={{ fontFamily: "'Caveat', cursive", fontSize: 24, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ display: 'inline-flex', width: 30, height: 30, border: 'var(--pmq-border)', borderRadius: 8, background: 'var(--pmq-accent)', color: 'white', alignItems: 'center', justifyContent: 'center', fontFamily: "'JetBrains Mono', monospace", fontSize: 13, boxShadow: 'var(--pmq-shadow-hard-sm)' }}>P!</span>
              PMQuest
            </div>
            <div className="nav">
              <span className="active-tab">Сегодня</span>
              <span>Все темы</span>
              <span>Mock</span>
              <span>Профиль</span>
            </div>
        ...
        <div className="stage">
          <div>
            <div className="mission">
              <div className="eyebrow">
                <span className="pmq-chip" style={{ background: 'var(--pmq-accent)', color: 'white', borderColor: 'var(--pmq-ink)' }}>⚑ миссия дня</span>
                <span className="pmq-chip">35 мин</span>
                <span className="pmq-chip">+120 XP</span>
                <span className="pmq-chip" style={{ background: 'var(--pmq-hl-pink)' }}>FAANG product design</span>
              </div>
              <h1>Сегодня —<br />один кейс. До конца.</h1>
              <p className="lede">Не открывай новых вкладок. План на сегодня — один кейс, обратная связь от AI, короткий behavioral. Достаточно, чтобы держать стрик и закрыть слабое место недели.</p>

              <div className="case-card">
                <div className="meta-row">
                  <span className="pmq-chip">кейс · 15 мин</span>
                  <span className="pmq-chip">+60 XP</span>
                  <span className="pmq-chip">слабая тема: product design</span>
                </div>
                <h3>Spotify хочет добавить «лайки» песен в плейлистах. Спроектируй.</h3>
                <div className="mono" style={{ color: 'var(--pmq-ink-3)', marginBottom: 6 }}>шаги · 5 · ai подсказывает по запросу</div>
                <div className="pmq-ph">диаграмма шагов кейса (clarify → user → pain → solutions → metrics)</div>
                <div style={{ display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' }}>
                  <button className="pmq-btn primary" onClick={startMock}>▶ Старт миссии</button>
                  <button className="pmq-btn">Пример сильного ответа</button>
                  <button className="pmq-btn ghost">Заменить кейс</button>
                </div>
              </div>

              <div className="secondary">
                <div className="mini">
                  <span className="label-tag">Шаг 2 · 12 мин</span>
                  <h4>Behavioral mock с AI</h4>
                  <p>«Tell me about a time you disagreed with a teammate.» AI задаст 3 вопроса и даст фидбек по STAR.</p>
                  <button className="pmq-btn sm">открыть</button>
                </div>
                <div className="mini">
                  <span className="label-tag">Шаг 3 · 8 мин</span>
                  <h4>Закрепи теорию</h4>
                  <p>5 флешкарт по продуктовым метрикам (DAU, retention, north star).</p>
                  <button className="pmq-btn sm" onClick={onOpenLearn}>открыть</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rail">
            <div className="card" style={{ background: 'var(--pmq-hl-yellow)' }}>
              <h4>🔥 Стрик <small className="mono" style={{ fontSize: 11 }}>7 / 30</small></h4>
              <div className="pmq-streak-grid">
                {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                  <div key={i} className={`day ${i === 6 ? 'today' : 'on'}`}>{d}</div>
                ))}
              </div>
              <p className="mono" style={{ margin: '8px 0 0', color: 'var(--pmq-ink-2)' }}>не теряй сегодня — выходные считаются ×1.5</p>
            </div>

            <div className="card">
              <h4>Прогресс к цели</h4>
              <div style={{ fontFamily: "'Patrick Hand', cursive", fontSize: 16, marginBottom: 6 }}>Junior PM @ FAANG</div>
              <div className="pmq-bar" style={{ marginBottom: 6 }}><i style={{ width: '46%' }} /></div>
              <div className="mono" style={{ marginBottom: 8 }}>46% готовности · 54 дня до цели</div>
              <hr className="pmq-hr-dash" />
              <div className="mono" style={{ marginBottom: 4 }}>слабое место недели:</div>
              <span className="pmq-chip" style={{ background: 'var(--pmq-hl-pink)' }}>Product design кейсы</span>
            </div>

            <div className="card" style={{ background: 'var(--pmq-hl-sky)' }}>
              <h4>AI-тутор Джо</h4>
              <div style={{ fontFamily: "'Patrick Hand', cursive", fontSize: 15, marginBottom: 8 }}>
                «Сделай миссию — и закрой вкладку. Если застрянешь, я рядом 👉»
              </div>
              <div className="pmq-ph" style={{ height: 60 }}>маскот / иллюстрация</div>
              <button className="pmq-btn sm" style={{ marginTop: 10 }}>спросить →</button>
            </div>

            <div className="card">
              <h4>Хочешь больше? <small className="mono" style={{ fontSize: 10 }}>опц.</small></h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <button className="pmq-chip" style={{ justifyContent: 'flex-start' }}>📘 банк FAANG-вопросов</button>
                <button className="pmq-chip" style={{ justifyContent: 'flex-start' }}>✎ CV ревью</button>
                <button className="pmq-chip" style={{ justifyContent: 'flex-start' }}>⌬ system design для PM</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <p className="hand" style={{ fontSize: 20, margin: '18px 0 0', color: 'var(--pmq-ink-2)', maxWidth: '80ch' }}>
        ➜ Сильно: ноль паралича выбора, легко начать, дисциплинирует. Минус: пользователю с лишним временем может стать скучно.
      </p>
    </section>
  );
}
...
/* W4 — Bento */
function PMQPanel4({ startMock, onOpenLearn, onOpenInterview, onOpenQuiz }) {
  return (
    <section>
      <div style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <div className="label-tag">VARIATION 04 / BENTO</div>
          <h2 className="hand" style={{ margin: '2px 0 0', fontSize: 30 }}>Bento-сетка — главное в плитках</h2>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-sky)' }} />современно, плотно</span>
          <span className="pmq-chip"><span className="dot" style={{ background: 'var(--pmq-hl-mint)' }} />хорошо для разных типов</span>
        </div>
      </div>

      <div className="pmq-frame pmq-w4">
        <span className="pmq-frame-caption">04 · Bento grid</span>
        <span className="pmq-frame-note">first impression: «вот мои инструменты»</span>
        ...
        <div className="topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
            ...
            <div style={{ display: 'flex', gap: 18, fontFamily: "'Patrick Hand', cursive", color: 'var(--pmq-ink-3)' }}>
              <span style={{ color: 'var(--pmq-ink)', borderBottom: '2px solid var(--pmq-accent)', paddingBottom: 2 }}>Home</span>
              <span style={{ cursor: 'pointer' }} onClick={onOpenLearn}>Темы</span>
              <span style={{ cursor: 'pointer' }} onClick={onOpenInterview}>Mock</span>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div className="pmq-chip" style={{ background: 'var(--pmq-hl-coral)' }}>🔥 7</div>
            <div className="pmq-chip">✦ 1 240 XP</div>
            <div className="pmq-chip">Lvl 4</div>
            <div className="pmq-avatar">EК</div>
          </div>
        </div>

        <div className="pmq-bento">
          <div className="pmq-tile t-hero">
            <span className="corner">primary action</span>
            <span className="label-tag" style={{ color: 'rgba(255,255,255,0.85)' }}>МИССИЯ ДНЯ · 35 МИН</span>
            <h2>Кейс<br />«Spotify — фича лайков»</h2>
            <p>Один кейс + behavioral mock + флешкарты. AI-тутор подсказывает по запросу.</p>
            <div className="footer">
              <span style={{ fontFamily: "'Patrick Hand', cursive", fontSize: 16 }}>+120 XP · слабое место недели</span>
              <button className="pmq-btn" style={{ background: 'white', color: 'var(--pmq-ink)' }} onClick={startMock}>▶ старт</button>
            </div>
          </div>

          <div className="pmq-tile t-streak">
            <span className="corner">streak</span>
            <h3>🔥 7-дневный стрик</h3>
            <div className="pmq-streak-grid" style={{ marginTop: 'auto' }}>
              {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                <div key={i} className={`day ${i === 6 ? 'today' : 'on'}`}>{d}</div>
              ))}
            </div>
          ...
          <div className="pmq-tile t-xp">
            <span className="corner">level 4 → 5</span>
            <h3>1 240 XP</h3>
            <p>+260 до следующего уровня</p>
            <div className="pmq-bar" style={{ marginTop: 'auto' }}><i style={{ width: '62%' }} /></div>
            <div className="mono" style={{ marginTop: 4 }}>62%</div>
          </div>

          <div className="pmq-tile t-faang">
            <span className="corner">faang bank</span>
            <h3>FAANG вопросы</h3>
            <p>820 вопросов из реальных интервью. Фильтры по компаниям и уровням.</p>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
              <span className="pmq-chip">Google · 142</span>
              <span className="pmq-chip">Meta · 110</span>
              <span className="pmq-chip">Amazon · 188</span>
              <span className="pmq-chip">Apple · 76</span>
              <span className="pmq-chip">Netflix · 38</span>
            </div>
            <div className="footer">
              <span className="mono">фильтры → product / behavioral / analytics</span>
              <button className="pmq-btn sm" onClick={onOpenQuiz}>открыть →</button>
            </div>
          </div>

          <div className="pmq-tile t-cases">
            <span className="corner">cases</span>
            <h3>Кейсы</h3>
            <p>Product design · estimation · strategy</p>
            <div className="footer">
              <div className="pmq-bar" style={{ width: 100 }}><i style={{ width: '62%' }} /></div>
              <button className="pmq-btn sm" onClick={startMock}>→</button>
            </div>
          </div>

          <div className="pmq-tile t-behav">
            <span className="corner">behavioral</span>
            <h3>STAR & behavioral</h3>
            <p>30 шаблонных вопросов + ваш банк историй</p>
            <div className="footer">
              <div className="pmq-bar" style={{ width: 100 }}><i style={{ width: '38%', background: 'var(--pmq-accent-2)' }} /></div>
              <button className="pmq-btn sm">→</button>
            </div>
          </div>

          <div className="pmq-tile t-cv">
            <span className="corner">cv review</span>
            <h3>CV ревью</h3>
            <p>Загрузи резюме — AI даст разбор + 3 правки</p>
            <div className="footer">
              <span className="mono">PDF / DOCX</span>
              <button className="pmq-btn sm primary">загрузить</button>
            </div>
          </div>

          <div className="pmq-tile t-tutor">
            <span className="corner" style={{ color: 'rgba(255,255,255,0.7)' }}>ai tutor</span>
            <h3>AI-тутор Джо</h3>
            <p>Задай вопрос по любой теме — отвечу с примерами из FAANG-интервью.</p>
            <div className="footer">
              <span className="mono">всегда онлайн</span>
              <button className="pmq-btn sm" style={{ background: 'white', color: 'var(--pmq-ink)' }} onClick={onOpenInterview}>спросить</button>
            </div>
          </div>

          <div className="pmq-tile t-board">
            <span className="corner">community</span>
            <h3>Сегодня учатся</h3>
            <p>Лидеры недели + люди, которые решают тот же кейс</p>
            <div className="pmq-stack-bars" style={{ marginTop: 'auto' }}>
              {[30, 55, 70, 45, 90, 60, 75, 40, 85].map((h, i) => <i key={i} style={{ height: `${h}%` }} />)}
            </div>
          </div>

          <div className="pmq-tile t-plan">
            <span className="corner">roadmap</span>
            <h3>Твой 8-недельный план</h3>
            <p>Foundations → Cases → Behavioral → System design → Analytics → Mocks → Финальная репетиция</p>
            <div style={{ display: 'flex', gap: 0, alignItems: 'center', marginTop: 'auto' }}>
              {[
                { label: '✓ нед.1', bg: 'var(--pmq-hl-mint)' },
                { label: '✓ нед.2', bg: 'var(--pmq-hl-mint)' },
                { label: '▶ нед.3', bg: 'var(--pmq-accent)', color: 'white' },
                { label: 'нед.4', bg: undefined },
                { label: 'нед.5', bg: undefined },
                { label: 'нед.6', bg: undefined },
                { label: 'нед.7', bg: undefined },
                { label: 'нед.8', bg: undefined },
              ].map((step, i) => (
                <React.Fragment key={i}>
                  <span className="pmq-chip" style={{ background: step.bg, color: step.color, borderColor: step.color ? 'var(--pmq-ink)' : undefined }}>{step.label}</span>
                  {i < 7 && <span style={{ flex: 1, height: 0, borderTop: '1.6px dashed var(--pmq-ink)', margin: '0 6px' }} />}
                </React.Fragment>
              ))}
            </div>
          </div>
      ...
      <p className="hand" style={{ fontSize: 20, margin: '18px 0 0', color: 'var(--pmq-ink-2)', maxWidth: '80ch' }}>
        ➜ Сильно: модульно, легко наращивать новыми плитками, выглядит «продакт-богато». Минус: без чёткой иерархии глаза разбегаются.
      </p>
    </section>
  );
}
...
/* W5 — Mechanics */
function PMQPanel5({ startMock, onOpenInterview }) {
  return (
    <section>
      <div style={{ marginBottom: 14 }}>
        <div className="label-tag">VARIATION 05 / MECHANICS — приколюхи</div>
        <h2 className="hand" style={{ margin: '2px 0 0', fontSize: 30 }}>Фичи, переосмысленные под PM-подготовку</h2>
        <p style={{ margin: '4px 0 0', color: 'var(--pmq-ink-2)', maxWidth: '88ch' }}>
          Идеи из EdTech-сервисов — но адаптированные под продуктовые интервью: фреймворки вместо формул, кейсы вместо задач, FAANG-интервьюер вместо учителя.
        </p>
      </div>

      <div className="pmq-w5-grid">
        {/* F1: Drill Mode */}
        <div className="pmq-feat">
          <span className="feat-tag accent">F1 · Drill Mode</span>
          <h3>Серия вопросов за 60 секунд каждый <small className="mono" style={{ fontSize: 11, color: 'var(--pmq-ink-3)' }}>flow-режим</small></h3>
          <p className="feat-sub">Быстрый «спринт» по слабому навыку — отвечаешь голосом или текстом, AI считает время и ставит звёзды.</p>
          <div className="pmq-drill-banner">
            <div className="left">
              <div className="icon-orb">⚡</div>
              <div>
                <h4>Готов(а) к drill-режиму?</h4>
                <p>10 product-sense вопросов · 60 сек на каждый</p>
              </div>
            </div>
            <button className="pmq-btn primary" onClick={startMock}>Start ▸</button>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
            <span className="pmq-chip">📊 Estimation drill</span>
            <span className="pmq-chip">📐 Trade-offs drill</span>
            <span className="pmq-chip">🎯 Metrics drill</span>
            <span className="pmq-chip">🗣 Behavioral 60s</span>
          </div>
          <div className="ref">→ <b>идея:</b> rapid-fire без перфекционизма — главное привычка думать вслух.</div>
        </div>

        {/* F2: Skill MCQ */}
        <div className="pmq-feat">
          <span className="feat-tag">F2 · Skill question + «Почему?»</span>
          <h3>MCQ-вопрос с разбором фреймворка</h3>
          <p className="feat-sub">Один правильный ответ — но Explain показывает, какой фреймворк применить.</p>
          <div className="pmq-skill-q">
            <div className="qhead">
              <b style={{ fontFamily: "'Caveat', cursive", fontSize: 22 }}>Q 3</b>
              <span className="ribbon">Product sense</span>
              <span className="spacer" />
              <div className="icon-btn green">✓</div>
              <div className="icon-btn yellow">🔖</div>
              <div className="icon-btn">⤢</div>
            </div>
            <p className="qbody">Какая метрика лучше всего отражает успех новой функции «лайков» в Spotify-плейлистах?</p>
            <div className="opt wrong">
              <div className="letter">A</div>
              <div>Общее число лайков в день</div>
              <span className="why">почему нет?</span>
            </div>
            <div className="opt">
              <div className="letter">B</div>
              <div>Среднее число лайков на пользователя в неделю</div>
            </div>
            <div className="opt right">
              <div className="letter">C</div>
              <div>% активных слушателей, поставивших ≥ 1 лайк за 30 дней</div>
              <span className="why" style={{ background: 'var(--pmq-hl-mint)' }}>разобрать ✓</span>
            </div>
            <div className="opt">
              <div className="letter">D</div>
              <div>Retention слушателей плейлистов с лайками</div>
            </div>
          </div>
          <div className="ref">→ <b>идея:</b> кнопка «почему нет?» убивает гадание.</div>
        </div>

        {/* F3: Teach the Rookie */}
        <div className="pmq-feat">
          <span className="feat-tag">F3 · Teach the Rookie</span>
          <h3>Объясни джуну — найди свои пробелы</h3>
          <p className="feat-sub">Feynman: AI прикидывается стажёром, ты объясняешь NSM или RICE. Дурацкие вопросы — там, где плывёшь.</p>
          <div className="pmq-rookie-chat">
            <div className="objective">
              <span className="o-mark">○</span>
              <span><b>Цель сессии:</b> объясни <i>North Star Metric</i> так, чтобы стажёр понял с нуля.</span>
            </div>
            <div className="msg">
              <div className="av">🐣</div>
              <div className="bubble">
                Подожди, а зачем нужна <b>одна</b> метрика? У нас же DAU, retention, revenue, NPS — почему бы не следить за всеми?
              </div>
            </div>
            <div className="msg user">
              <div className="av">EК</div>
              <div className="bubble" style={{ background: 'var(--pmq-hl-yellow)' }}>
                Окей, North Star — это <mark>главный показатель ценности продукта для пользователя</mark>, к которому прицепляются остальные…
              </div>
            </div>
            <div className="msg">
              <div className="av">🐣</div>
              <div className="bubble">
                А чем тогда она отличается от обычной «целевой метрики бизнеса»? И как её выбрать, если у нас 5 равных кандидатов?
              </div>
            </div>
            <div className="compose">
              <span>Объясни на пальцах…</span>
              <span className="send">▸</span>
            </div>
          </div>
          <div className="ref">→ <b>идея:</b> «учитель» вместо «ученика» — недооценённый способ готовиться.</div>
        </div>

        {/* F4: Spaced repetition */}
        <div className="pmq-feat">
          <span className="feat-tag">F4 · Spaced repetition · фреймворки</span>
          <h3>CIRCLES, AARM, RICE… в карточках <small className="mono" style={{ fontSize: 11, color: 'var(--pmq-ink-3)' }}>15 / 240</small></h3>
          <p className="feat-sub">Не «зубришь определения», а вспоминаешь их в нужные интервалы.</p>
          <div className="pmq-srs-card">
            <div className="srs-tag">Фреймворк · product design кейс</div>
            <div className="srs-q">CIRCLES — что значит каждая буква?</div>
            <div className="srs-a">переверни (Spacebar) — увидишь и пример применения</div>
          </div>
          <div style={{ fontFamily: "'Patrick Hand', cursive", textAlign: 'center', marginBottom: 8, color: 'var(--pmq-ink-3)', fontSize: 14 }}>Как запомнил?</div>
          <div className="pmq-srs-buttons">
            <div className="b again"><div className="l">Снова</div><div className="t">10 мин</div></div>
            <div className="b hard"><div className="l">Тяжело</div><div className="t">45 мин</div></div>
            <div className="b good"><div className="l">Нормально</div><div className="t">1 день</div></div>
            <div className="b easy"><div className="l">Легко</div><div className="t">4 дня</div></div>
          </div>
          <div className="ref">→ <b>идея:</b> терминология PM = вокабуляр. Тренируй как иностранный язык.</div>
        </div>

        {/* F5: Mock Wizard */}
        <div className="pmq-feat">
          <span className="feat-tag">F5 · Mock Setup Wizard</span>
          <h3>3 шага → твой mock</h3>
          <p className="feat-sub">Компания · уровень · фокус.</p>
          <div className="pmq-wiz">
            <div className="wiz-head">
              <span className="step">Step 2 of 3</span>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--pmq-ink-3)' }}>× закрыть</span>
            </div>
            <h4 className="wiz-q">Что прокачиваем сегодня?</h4>
            <div className="opt-list">
              <div className="opt-row checked"><span className="box">✓</span> Product design кейсы</div>
              <div className="opt-row checked"><span className="box">✓</span> Estimation / guesstimates</div>
              <div className="opt-row"><span className="box" /> Behavioral / STAR</div>
              <div className="opt-row"><span className="box" /> Metrics &amp; A/B</div>
              <div className="opt-row"><span className="box" /> System design</div>
            </div>
            <div className="wiz-foot">
              <button className="pmq-btn sm ghost">‹ Назад</button>
              <button className="pmq-btn sm primary" onClick={onOpenInterview}>Дальше ›</button>
            </div>
          </div>
          <div className="ref">→ <b>идея:</b> mock = ритуал; чем меньше выбор, тем чаще приходишь.</div>
        </div>

        {/* F6: Persona */}
        <div className="pmq-feat">
          <span className="feat-tag">F6 · Interviewer Persona</span>
          <h3>Кто будет «гонять» сегодня?</h3>
          <p className="feat-sub">Уровень дружелюбия AI-интервьюера.</p>
          <div className="pmq-persona-grid">
            <div className="pmq-persona">
              <div className="face friend">😊</div>
              <h5>Friendly</h5>
              <p>помогает<br />подсказывает</p>
            </div>
            <div className="pmq-persona active">
              <div className="face std">🧐</div>
              <h5>Standard</h5>
              <p>как на<br />реальном интервью</p>
            </div>
            <div className="pmq-persona">
              <div className="face aggro">😤</div>
              <h5>Aggressive</h5>
              <p>FAANG hard mode<br />прессует</p>
            </div>
          </div>
          <hr className="pmq-hr-dash" />
          <div style={{ fontFamily: "'Patrick Hand', cursive", color: 'var(--pmq-ink-3)', marginBottom: 6, fontSize: 13 }}>Длительность:</div>
          <div style={{ display: 'flex', gap: 6 }}>
            <span className="pmq-chip">15 мин</span>
            <span className="pmq-chip" style={{ background: 'var(--pmq-hl-yellow)' }}>30 мин</span>
            <span className="pmq-chip">45 мин</span>
          </div>
          <div className="ref">→ <b>идея:</b> страх FAANG-интервью лечится дозированной экспозицией.</div>
        </div>

        {/* F7: AI prompts */}
        <div className="pmq-feat">
          <span className="feat-tag">F7 · Quick AI prompts</span>
          <h3>«Спроси одной кнопкой»</h3>
          <p className="feat-sub">После ответа на кейс — готовые промпты для разбора.</p>
          <div className="pmq-ai-prompts">
            {[
              'Сделай мой ответ сильнее',
              'Какой follow-up задал бы Google?',
              'Где я звучал(а) как джун?',
              'Покажи 3 разных подхода к этому кейсу',
              'Оцени по PM-бару Meta',
              'Сократи до 2-минутного ответа',
            ].map((p) => (
              <div key={p} className="prompt">{p} <span className="arrow">↩</span></div>
            ))}
          </div>
          <div className="ref">→ <b>идея:</b> сила не в чате, а в правильно сформулированном запросе.</div>
        </div>

        {/* F8: Notes */}
        <div className="pmq-feat">
          <span className="feat-tag">F8 · Заметки с подсветкой + Ask Coach</span>
          <h3>Конспекты с inline-разбором</h3>
          <p className="feat-sub">Выделяешь любой кусок → мини-меню: подсветить, спросить тренера, добавить в свой банк.</p>
          <div className="pmq-notes-doc">
            <h5>Что такое North Star Metric и как её выбрать?</h5>
            <div className="def-callout">
              <b>определение</b><br />
              Одна метрика, которая лучше всего отражает <b>ценность</b>, получаемую пользователем от продукта.
            </div>
            <ol style={{ margin: 0, paddingLeft: 22 }}>
              <li>NSM — это <mark>не про деньги напрямую</mark>, а про реализованную ценность.</li>
              <li>Хорошая NSM связана с retention и в долгую коррелирует с revenue.</li>
              <li><mark>Airbnb: «ночей забронировано»</mark>. Facebook: «MAU 7-day-engaged». Spotify: «время прослушивания».</li>
              <li>Плохая NSM: суммарный трафик, разовые клики, лайки.</li>
              <li>Проверь себя: <mark>если NSM растёт, пользователь точно счастливее?</mark></li>
            </ol>
            <span className="floater">
              <span className="pill accent">🖍 подсветить</span>
              <span className="pill">＋ в банк историй</span>
              <span className="pill coach">🥋 спроси Coach</span>
            </span>
          </div>
          <div className="ref">→ <b>идея:</b> заметки = живой документ, а не PDF на полке.</div>
        </div>

        {/* F9: Resume mock */}
        <div className="pmq-feat">
          <span className="feat-tag accent">F9 · Resume your mock</span>
          <h3>Продолжи с того же места</h3>
          <p className="feat-sub">Mock-интервью pause-able. Откладываешь — возвращаешься на той же реплике.</p>
          <div className="pmq-resume-mock">
            <div>
              <div className="timer">прервано · вчера · 14:32 · осталось 12 мин</div>
              <h5>Mock: Google · Product Design</h5>
              <p>«Design a Maps experience for kids 8–12». Дошёл(шла) до шага <b>3 / 5 · Solutions</b>.</p>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <button className="pmq-btn sm primary" onClick={startMock}>Продолжить</button>
                <button className="pmq-btn sm ghost">Сначала</button>
                <button className="pmq-btn sm ghost">Завершить</button>
              </div>
            </div>
            <div className="pmq-ph">кадр<br />интервьюера</div>
          </div>
          <hr className="pmq-hr-dash" />
          <div style={{ fontFamily: "'Caveat', cursive", fontSize: 22, marginBottom: 4 }}>Score-черновик пока ты думала</div>
          <div className="pmq-score-rows">
            {[
              { name: 'Clarifying Qs', dots: 4, val: '4/5' },
              { name: 'User & pain', dots: 3, val: '3/5' },
              { name: 'Solutions', dots: 2, val: '2/5' },
              { name: 'Metrics', dots: 0, val: '—' },
            ].map((r) => (
              <div className="row" key={r.name}>
                <div className="name">{r.name}</div>
                <div className="pmq-score-dots">
                  {[0, 1, 2, 3, 4].map((i) => (
                    <span key={i} className={`d${i < r.dots ? ' on' : ''}`} />
                  ))}
                </div>
          ...
          <div className="ref">→ <b>идея:</b> убери трение «нужно сесть на 60 минут».</div>
        </div>

        {/* F10: Live pace */}
        <div className="pmq-feat" style={{ columnSpan: 'all', WebkitColumnSpan: 'all' }}>
          <span className="feat-tag">F10 · Live pace-meter во время mock</span>
          <h3>Полоска темпа: где ты сейчас vs где должен быть</h3>
          <p className="feat-sub">Во время живого mock видишь «дорожку» — на каком шаге кейса должен быть на этой минуте. Без неё джуны 80% времени тратят на clarifying questions.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 6, fontFamily: "'Patrick Hand', cursive", fontSize: 14, textAlign: 'center' }}>
            {[
              { label: '1 · Clarify', t: '2 мин' },
              { label: '2 · User & pain', t: '3 мин' },
              { label: '3 · Solutions', t: '4 мин' },
              { label: '4 · Priorities', t: '3 мин' },
              { label: '5 · Metrics', t: '3 мин' },
            ].map((s) => (
              <div key={s.label}>{s.label} <small className="mono" style={{ display: 'block', fontSize: 10, color: 'var(--pmq-ink-3)' }}>{s.t}</small></div>
            ))}
          </div>
          <div className="pmq-race-bar">
            <i style={{ width: '42%' }} />
            {[13.3, 33.3, 60, 80].map((pos) => (
              <span key={pos} className="tick" style={{ left: `${pos}%` }} />
            ))}
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'var(--pmq-ink-3)' }}>
            <span>00:00</span>
            <span style={{ color: 'var(--pmq-accent)' }}>▶ ты тут · 6:18 · застрял на «solutions»</span>
            <span>15:00</span>
          </div>
          <div style={{ marginTop: 14, display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <span className="pmq-chip" style={{ background: 'var(--pmq-hl-coral)' }}>тратишь много на clarify (+1:30 к плану)</span>
            <span className="pmq-chip" style={{ background: 'var(--pmq-hl-mint)' }}>в metrics ещё успеваешь</span>
            <span className="pmq-chip" style={{ background: 'var(--pmq-accent)', color: 'white', borderColor: 'var(--pmq-ink)' }}>📣 AI шепнёт, если выходишь за лимит</span>
          </div>
          <div className="ref">→ <b>идея:</b> джуны проигрывают интервью на тайминге чаще, чем на содержании.</div>
        </div>
      </div>

      <div style={{ marginTop: 22, padding: '14px 18px', border: '1.5px dashed var(--pmq-ink-4)', borderRadius: 12, background: 'var(--pmq-paper-2)' }}>
        <div className="label-tag">КАК СОБИРАЕТСЯ В ПРОДУКТ</div>
        <p className="hand" style={{ fontSize: 20, margin: '6px 0 0', color: 'var(--pmq-ink-2)', maxWidth: '90ch' }}>
          F1, F4, F7 — встраиваются в любой дашборд (механики на каждый день).
          F5, F6, F9, F10 — формируют отдельный экран «Mock Room».
          F2, F8 — на экранах «Banks / Theory». F3 — отдельный режим 2–3 раза в неделю.
        </p>
      </div>
    </section>
  );
