...
def _case_prompt(payload: GenerateCaseRequest) -> tuple[str, str]:
    ...
    if payload.mvp:
        prompt += "\nКейс на 15–20 минут. Дай короткий заголовок, условие, достаточные численные данные и конкретный вопрос. Решение строится через восемь коротких вопросов: цель, целевой сегмент, гипотеза, факт из условия, первая проверка, действие, метрика успеха, риск. Оформи условие короткими разделами: Контекст, Проблема, Данные, Ограничения, Задача. Данные покажи списком или таблицей. Не добавляй учебный маршрут, ответы или подсказки. Все данные вымышленные, но внутренне согласованные."
...
def evaluate(payload: EvaluateRequest) -> EvaluateResponse:
    if payload.mvp:
        fields = {
            ...
            "audience": "Для кого это особенно важно?",
            "hypotheses": "Гипотезы",
            "analysis": "Данные и расчёты",
            "validation": "Что проверишь первым?",
