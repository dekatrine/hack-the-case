...
def call_yandex_gpt(
    ...
    """Вызов YandexGPT Foundation Models API с retry/backoff."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning("YandexGPT timeout, attempt %s/%s", attempt + 1, max_retries)
        except requests.exceptions.HTTPError:
            status = resp.status_code if "resp" in locals() else "?"
            last_error = f"HTTP {status}"
            logger.warning("YandexGPT HTTP error %s, attempt %s/%s", status, attempt + 1, max_retries)
            if status != 429 and isinstance(status, int) and 400 <= status < 500:
                break
        except (KeyError, IndexError, json.JSONDecodeError) as e:
            last_error = f"неожиданный ответ API: {e}"
            logger.warning("YandexGPT malformed response: %s", e)
            break
        except Exception as e:
            last_error = str(e)
            logger.exception("YandexGPT error, attempt %s/%s", attempt + 1, max_retries)
...
CASE_HINTS = {
    "issue_tree": "Внимательно прочитай раздел **Проблема** в условии кейса. Выдели главную проблему и разложи её на подпроблемы. Проверь, что ветки не пересекаются и вместе покрывают задачу.",
    "research": "Обрати внимание на раздел **Контекст**: тренды, конкуренция, позиция компании. Для market sizing отдельно пометь, где есть данные из кейса, а где твои допущения.",
    "segmentation": "Из разделов **Компания** и **Контекст** вытащи клиентов, пользователей и плательщиков. Сегментируй не только по демографии, но и по потребности, поведению или JTBD.",
    "cjm": "Опиши путь клиента от первого контакта до повторного использования. Для каждого этапа добавь touchpoint, pain point и возможную метрику конверсии.",
    "initiatives": "Вернись к корневым причинам из issue tree. Каждая инициатива должна отвечать на конкретную причину и иметь понятный impact/effort.",
    "metrics": "Начни с North Star Metric, затем разложи её на драйверы. Для экспериментов заранее зафиксируй гипотезу, критерий успеха и срок проверки.",
    "economics": "Свяжи финмодель с данными кейса: выручкой, клиентами, бюджетом, сроками. Если цифры оценочные, явно подпиши допущения.",
    "risks": "Раздели риски на рыночные, операционные, финансовые и репутационные. Для ключевых рисков укажи probability, impact и митигацию.",
    "roadmap": "Собери roadmap из инициатив: quick wins, зависимости, владельцы, сроки и checkpoints. Проверь, что план реалистичен при ограничениях из кейса.",
}
...
def build_previous_steps_summary(step_answers: dict, current_step_id: str) -> str:
    """Кратко собирает ответы предыдущих этапов для контекста коуча."""
    summary_parts = []
...
def build_coach_context(step: dict, answer_text: str, chat_history: list, user_message: str) -> str:
    """Формирует контекст коуча: кейс, текущий шаг, предыдущие ответы и историю чата."""
    prev_summary = build_previous_steps_summary(st.session_state.step_answers, step["id"])
    recent_history = "\n".join(
        f"{'Студент' if msg['role'] == 'student' else 'Коуч'}: {msg['text']}"
        for msg in chat_history[-6:]
    )

    context = f"""Кейс:
{st.session_state.case_text[:2500]}

Текущий этап: {step['title']}
Цель этапа: {step['description']}
Рекомендуемые фреймворки: {', '.join(step['frameworks'])}
Подсказка по этапу: {CASE_HINTS.get(step['id'], 'нет')}
"""
...
Последние сообщения:
{recent_history if recent_history else '(истории пока нет)'}
...
def parse_rubric_json(raw_text: str) -> dict | None:
    """Пробует извлечь JSON из ответа жюри, если модель вернула структурированный результат."""
    try:
...
def render_evaluation(evaluation: str):
    """Показывает оценку как структурный отчёт, если удалось распарсить JSON."""
    parsed = parse_rubric_json(evaluation)
    if not parsed:
        st.markdown(evaluation)
        return

    total_score = parsed.get("total_score") or parsed.get("score") or parsed.get("total")
    if total_score is not None:
        st.markdown(f'<div class="score-box">{total_score}/100</div>', unsafe_allow_html=True)

    criteria = parsed.get("criteria") or parsed.get("scores") or []
    if isinstance(criteria, dict):
        criteria = [{"name": key, **value} if isinstance(value, dict) else {"name": key, "score": value} for key, value in criteria.items()]

    if criteria:
        st.markdown("### Оценка по критериям")
        for item in criteria:
            name = item.get("name") or item.get("criterion") or "Критерий"
            score = item.get("score", "—")
            comment = item.get("comment") or item.get("explanation") or item.get("why") or ""
            action = item.get("recommendation") or item.get("fix_action") or item.get("fix_actions") or ""
    ...
    summary = parsed.get("summary") or parsed.get("resume") or parsed.get("overall_feedback")
    if summary:
        st.markdown("### Резюме")
        st.write(summary)
...
def page_solve():
    ...
    with col_work:
        ...
        st.info(CASE_HINTS.get(step["id"], "Сформулируй ответ так, чтобы его можно было проверить по данным кейса."))
...
По возможности верни валидный JSON без markdown-обёртки:
{{
  ...
  "criteria": [
    {{
      ...
      "recommendation": "конкретное действие для улучшения"
    }}
  ],
  "summary": "сильные стороны, зоны роста и 3 конкретных совета"
}}"""
