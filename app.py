...
def page_start():
    ...
    with col2:
        ...
        **2. Решение** — 9 этапов, как на кейс-чемпионате

        **3. AI-Coach** — задаёт вопросы и направляет, не решает за тебя

        **4. Оценка** — Rubric-жюри ставит баллы по 10 критериям
        """)
        st.markdown("---")
...
def page_solve():
    ...
    with col_coach:
        ...
        coach_input = st.text_input(
            ...
            placeholder="Например: 'Проверь мой issue tree' или 'Подскажи фреймворк'",
    ...
    with col1:
        if not st.session_state.evaluation:
            if filled == 0:
                ...
                st.info(f"Заполнено {filled} из {len(CASE_STEPS)} блоков.")
